...
// show how to use function indexOf and splice
...
for (i=0; i<mathScores.length; i++)
{
	if(mathScores[i].mathScore>=scoreLimit){    //comparing scores of mathscores
		topScorers.push(mathScores[i].name);//向数组加值
	}
}
console.log(topScorers);

for(j=0;j<englisScores.length;j++){		//matching names
		var lIndex = topScorers.indexOf(englisScores[j].name);
		if (lIndex >= 0 && englisScores[j].englishScore < scoreLimit){	//comparing scores of englishscores) {
			topScorers.splice(lIndex,1);			//splice:数组中删除元素(index,howmany(删除的个数))
		}
